...
from lfm_train.dataset import align_eos_token, ensure_chat_template, load_prompt_config
from lfm_train.evaluate import _infer


def load(adapter: str, max_seq_length: int = 512):
    """Load an adapter for inference. Returns (model, tokenizer)."""
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=adapter, max_seq_length=max_seq_length, load_in_4bit=True,
        dtype=None, use_exact_model_name=True,
    )
...
def _run_batch(model, tokenizer, prompts: list[dict], system_prompt: str,
               input_label: str, max_new_tokens: int) -> None:
    for p in prompts:
        result = _infer(model, tokenizer, p["instruction"], p.get("query", ""),
                        system_prompt, input_label, max_new_tokens=max_new_tokens)
        print(f"[{p.get('label', '')}]")
        print(f"  {result}\n")


def _run_repl(model, tokenizer, system_prompt: str, input_label: str,
    ...
    while True:
        ...
        result = _infer(model, tokenizer, instruction, input_q,
                        system_prompt, input_label, max_new_tokens=max_new_tokens)
        print(f"\n{result}\n")
...
def main() -> None:
    ...
    if args.prompts:
        ...
        _run_batch(model, tokenizer, prompts, system_prompt, input_label, args.max_new_tokens)
    else:
        _run_repl(model, tokenizer, system_prompt, input_label, args.max_new_tokens)
